...
class RealCommunicator(BaseCommunicator):
        def __init__(self, portname):
                print("Created RealCommunicator object.")
                # シリアル通信(USBに信号を送る)のインスタンスを作成
                self.ser = serial.Serial(portname, 115200, timeout=None)
                print(f"Created serial.Serial object.[portname={portname},portrait={115200},timeout=None]RealCommunicator")
...
class EV3():
        ...
        def motor_rotate(self, motor_port, derection, speed):
                ...
                self._write([motor_port, min(max(derection + 100, 0), 200), speed])
        
        def motor_set_power(self, motor_port, power):
                self._send_header(221)
                self._write([motor_port, min(max(power + 100, 0), 200)])
